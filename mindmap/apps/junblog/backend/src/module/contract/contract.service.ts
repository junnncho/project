...
export class ContractService extends LogService {
  ...
  constructor(@Inject('SOLANA_OPTIONS') private readonly options: SolanaOptions) {
  ...
  async addWithdraw(wallet: string, amount: number, transaction: web3.Transaction): Promise<web3.Transaction> {
    transaction.add(
  ...
  async withdraw(transaction: web3.Transaction): Promise<void> {
    ...
    await web3.sendAndConfirmTransaction(this.connection, transaction, [this.mainKey]);
  }
  ...
  async deposit(userPrivate: string): Promise<number> {
    ...
    if (balance > 0) {
      ...
      transaction.feePayer = this.mainKey.publicKey;
      try {
        await web3.sendAndConfirmTransaction(this.connection, transaction, [userKey, this.mainKey]);
      } catch (e) {
