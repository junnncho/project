...
@Schema()
class Base {
  // @Prop({ type: [ObjectId], ref: "video", required: true, default: [] })
  // videos: Id[];
...
class VideoView extends Tail {}
export const name = VideoView.name;
export type Raw = VideoView;
...
schema.statics.getLatestOriginId = async function () {
  ...
  if (!lastId) new Error('any story in VideoView DB');
  return lastId;
};
