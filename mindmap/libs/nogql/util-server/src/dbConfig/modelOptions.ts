...
export const getDefaultModelStatics = <TDocument, TSchema>(): DefaultMdlStats<
  TDocument,
  TSchema
> => ({
  ...
  pickById: async function (
    docId: Types.ObjectId | string | undefined
  ): Promise<TDocument> {
  ...
  addSummary: async function (prefix = 'total', num = 1): Promise<void> {
    const update = Array.isArray(prefix)
      ? {
          $inc: {
            ...prefix.reduce(
              (acc, cur) => ({ ...acc, [`${cur}${this.modelName}`]: num }),
              {}
            ),
          },
        }
    ...
    await this.db
      ...
      .updateOne({ status: 'active' }, update);
  },
  moveSummary: async function (
    prev: string,
    next: string,
    num = 1
  ): Promise<void> {
...
export const getDefaultQueryHelpers = <TDocument, TQryHelps>(): DefaultQryHelps<
  TDocument,
  TQryHelps
> => ({
