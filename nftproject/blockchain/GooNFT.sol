...
// File: contracts\KIP17GooNFT.sol

pragma solidity ^0.5.0;
contract KIP17GooNFT is KIP17, KIP17Enumerable, KIP17Metadata, MinterRole {
    ...
    uint256 private _teamNum;
    uint256 private _wlNum;
    uint256 private _ogNum;
    uint256 private _publicNum;
    uint256 private _mintLimitPerSale;            // Maximum purchase nft per person per sale

    string  private _tokenBaseURI;
    uint256 private _publicStartBlockNumber;        // In blockchain, blocknumber is the standard of time.
    uint256 private _wlStartBlockNumber;        // In blockchain, blocknumber is the standard of time.
...
function withdraw() external onlyMinter{
    ...
    function mintingInformation() external view returns (uint256[9] memory){
      uint256[9] memory info =
        [_mintIndexForSale,_ogNum,_wlNum,_publicNum,_teamNum, _mintLimitPerSale, 
          _publicStartBlockNumber,_wlStartBlockNumber, _mintPrice];
      return info;
    }
    ...
    function setupSale(
                       ...
                       uint256 newPublicStartBlockNumber,
                       uint256 newWLStartBlockNumber,
                       uint256 newMintIndexForSale,
                       uint256 newWLNum,
                       uint256 newPublicNum,
                       uint256 newOGNum,
                       uint256 newTeamNum,
                       uint256 newMintPrice) external onlyMinter{
      _mintLimitPerSale = newMintLimitPerSale;
      _publicStartBlockNumber = newPublicStartBlockNumber;
      _wlStartBlockNumber = newWLStartBlockNumber;
      _mintIndexForSale = newMintIndexForSale;
      _wlNum = newWLNum;
      _ogNum = newOGNum;
      _teamNum = newTeamNum;
      _publicNum = newPublicNum;
      _mintPrice = newMintPrice;
    }


    //WL & OG
    bytes32 public merkleOGRoot;
    bytes32 public merkleWLRoot;
    mapping(address => bool) public wlClaimed;
    mapping(address => bool) public ogClaimed;
    mapping(address => bool) public publicClaimed;

    function setMerkleOGRoot(bytes32 _merkleOGRoot) public onlyMinter {
      merkleOGRoot = _merkleOGRoot;
    }
    function setMerkleWLRoot(bytes32 _merkleWLRoot) public onlyMinter {
      merkleWLRoot = _merkleWLRoot;
    }
    function ChangeMintMode(uint8 _mode) public onlyMinter {
        mintMode = _mode;
    }

    //Public Mint
    function publicMint(uint256 requestedCount) external payable {
      require(mintMode == 3, "not enabled!");
      require(block.number >= _publicStartBlockNumber, "Not yet started");
      require(requestedCount > 0 && requestedCount <= 1, "Too many requests");
      require(msg.value == _mintPrice.mul(requestedCount), "Not enough");
      require(!publicClaimed[msg.sender], 'Address already claimed!');
      require(_mintIndexForSale <= _ogNum.add(_wlNum).add(_publicNum), "Exceed max");
    ...
    function OGMint(uint256 requestedCount, bytes32[] calldata _merkleProof) external payable {
      require(mintMode == 1, "not enabled");
      require(msg.value == _mintPrice.mul(requestedCount), "Not enough");
      require(!ogClaimed[msg.sender], 'Address already claimed!');
      require(requestedCount > 0 && requestedCount <= 2, "Too many requests");
      require(_mintIndexForSale <= _ogNum, "Exceed max");
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(_merkleProof, merkleOGRoot, leaf), 'Invalid proof!');
    ...
    function WLMint(uint256 requestedCount, bytes32[] calldata _merkleProof) external payable {
      require(mintMode == 2, "not enabled");
      require(block.number >= _wlStartBlockNumber, "Not yet started");
      require(msg.value == _mintPrice.mul(requestedCount), "Not enough Klay");
      
      require(!wlClaimed[msg.sender], 'Address already claimed!');
      require(requestedCount > 0 && requestedCount <= 3, "Too many requests");
      require(_mintIndexForSale <=_ogNum.add(_wlNum), "Exceed max");
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(_merkleProof, merkleWLRoot, leaf), 'Invalid proof!');
    ...
    function airDropMint(address user, uint256 requestedCount) external onlyMinter {
      require(mintMode == 4, "not enabled!");
      require(_mintIndexForSale <=_ogNum.add(_wlNum).add(_publicNum).add(_teamNum), "Exceed max");
 ...
 * @dev see http://kips.klaytn.com/KIPs/kip-17-non_fungible_token
 */
contract KIP17Full is KIP17, KIP17Enumerable, KIP17Metadata, Ownable, KIP17GooNFT {
...
// File: contracts\KIP17GooNFTToken.sol
...
contract KIP17GooNFTToken is KIP17Full, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable {
