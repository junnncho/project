  ...
  return new DataLoader<K, V>((fields: any) => {
    ...
    const data = model.find(query).then((list) => {
      return fields.map((field) =>
        list.filter((item) => (field as Types.ObjectId).equals(item[fieldName]))
      );
    });
    return data;
  });
  ...
  return new DataLoader<K, V>(
    (fields: any): any => {
      ...
      const data = populateFields
        .reduce(
          (fn: Query<unknown, unknown, unknown, unknown>, field: string) =>
            fn.populate(field),
          model.find(query)
        )
...
export const createArrayElementPopulatedLoader = <K, V>(
  ...
  return new DataLoader<K, V>(
    (fields) => {
      ...
      const data = populateFields
        .reduce(
          (fn: FilterQuery<unknown>, field: string) => fn.populate(field),
          model.find(query)
        )
        .then((list: Document<unknown>[]) => {
    ...
    (queries: any): any => {
      ...
      const data = model.find(query).then((list: Document<unknown>[]) => {
        ...
        return queries.map((query) =>
          _.get(listByKey, getQueryKey(query), null)
        );
      });
      return data;
    },
