...
export const randomCode = (length = 6) =>
  pad(Math.floor(Math.random() * 10 ** length), 6);
export const randomString = (length = 12) =>
  [
    ...new Array(Math.floor(length / 12))
      .fill(0)
      .map(() => (Math.random() + 1).toString(36).slice(2, 14)),
...
export const weightedPick = <T = unknown>(
  arr: T[],
  weights: number[],
  tWeight?: number
) => {
...
export const groupByFields = <
  TElement extends { [key: string]: string | number }
>(
  arr: TElement[],
  fieldName: string
): { [key: string]: TElement[] } => {
...
export const getBox = (
  ...
  const center: [number, number] = [
    Math.floor((edge.x + point[0]) / 2),
    Math.floor((edge.y + point[1]) / 2),
  ];
  return { center, wh };
};
...
export const objPath = <T>(o: T, p: string) =>
  p.split('.').reduce((a: any, v: string) => a && a[v], o);

export const doc2Xlsx = async (docs: any[], schema: any) => {
  const keys = Object.keys(schema);
  const array = [keys.map((key) => schema[key])];
  docs.forEach((doc) => {
    array.push(keys.map((key) => doc[key]));
  });
