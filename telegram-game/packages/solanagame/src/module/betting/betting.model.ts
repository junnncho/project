...
@Schema()
class Base {
  @Prop({ type: ObjectId, required: true, immutable: true })
  user: Id;

  @Prop({ type: ObjectId, required: true, immutable: true, index: true })
  game: Id;
...
class Tail extends Base {
  ...
  status: cnst.BettingStatus;
}

@Schema(dbConfig.defaultSchemaOptions)
class Betting extends Tail {}
export const name = Betting.name;
export type Raw = Betting;
...
export const schema: Sch<null, Mdl, DocMtds, QryHelps, null, MdlStats> =
  SchemaFactory.createForClass<Raw, Doc>(Betting) as any;
...
interface MdlStats extends dbConfig.DefaultMdlStats<Doc, Raw> {
  ...
  findByGameId: (gameId: Id) => Promise<Doc[]>;
}
...
schema.statics.getLatestOriginId = async function () {
  ...
  if (!lastId) new Error('any story in Betting DB');
  return lastId;
};

schema.statics.findByGameId = async function (gameId: Id) {
  const list = await this.find({ game: gameId });
  if (!list) throw new Error('no betting in this game');
  return list;
};
